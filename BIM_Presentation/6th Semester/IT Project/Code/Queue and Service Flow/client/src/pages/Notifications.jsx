...
const Notifications = () => {
  ...
  return (
    <MainLayout>
      <div className="max-w-4xl mx-auto py-3 sm:py-4 px-1 sm:px-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col xs:flex-row sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            <Button type="button" variant="secondary" className="w-full sm:w-auto justify-center" onClick={markAll} disabled={!unreadCount}>
              <CheckCheck className="h-4 w-4 mr-2" />
              Mark all as read
            </Button>
            <Button type="button" variant="secondary" className="w-full sm:w-auto justify-center" onClick={removeAll} disabled={!notifications.length && !unreadCount}>
        ...
        <div className="mt-5 sm:mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="grid grid-cols-3 rounded-xl bg-neutral-100 p-1 w-full sm:w-fit">
            {FILTERS.map((item) => (
              <button
                key={item.key}
                type="button"
                onClick={() => setFilter(item.key)}
                className={`px-3 sm:px-4 py-2.5 rounded-lg text-sm font-semibold transition text-center ${filter === item.key ? "bg-white shadow-sm text-primary-800" : "text-neutral-600 hover:text-neutral-900"}`}
              >
                {item.label}
              </button>
            ))}
          ...
          <select
            ...
            className="w-full sm:w-auto min-h-[44px] rounded-lg border border-neutral-300 bg-white px-3 text-sm text-neutral-700 focus:outline-none focus:ring-2 focus:ring-primary-300"
            ...
            <div className="space-y-3">
              {notifications.map((notification) => {
                ...
                return (
                  <article
                    key={notification.id}
                    className={`relative rounded-2xl border p-4 pr-12 sm:p-5 sm:pr-12 transition ${notification.read ? "border-neutral-200 bg-white" : "border-red-200 bg-red-50/60 shadow-sm"}`}
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-col gap-1 min-w-0">
                          <div>
                            <div className="flex items-center gap-2">
                              <h2 className={`font-semibold break-words ${notification.read ? "text-neutral-900" : "text-neutral-950"}`}>{title}</h2>
                              {!notification.read && <span className="h-2 w-2 rounded-full bg-red-600" aria-label="Unread" />}
                            </div>
                            <p className="text-sm text-neutral-600 mt-1 break-words leading-relaxed">{message}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0 mt-1 sm:mt-2">
                            ...
                            <button
                              type="button"
                              className="absolute right-2 top-2 sm:right-3 sm:top-3 inline-flex h-9 w-9 items-center justify-center rounded-full text-neutral-400 bg-white/80 border border-neutral-200 shadow-sm hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition z-10"
